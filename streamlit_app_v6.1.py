...
# --- 1. 页面与样式配置 ---
st.set_page_config(
    page_title="隧道工程检验批划分系统 Pro v9.6",
    page_icon="🚇",
    layout="wide",
...
def main():
    ...
    if page == "📋 参数配置":
        ...
        with col_basic:
            ...
            with st.form("basic_info"):
                ...
                # 若无段落，默认显示0，否则取段落两端的极值
                if not target_tunnel.segments:
                    st_val = 0.0; ed_val = 100.0
                else:
            ...
            if st.button("💾 保存段落 & 触发连缀推算", type="primary"):
                ...
                for idx, row in edited_df.iterrows():
                    try:
                        ...
                        prev_end_m = end_m # 将本段终点存入缓存，供下一循环使用
    ...
    # ===== 页面：统计看板 =====
        ...
        if 'last_result' in st.session_state:
            ...
            ax2.pie(total_series, labels=total_series.index, autopct='%1.1f%%', startangle=140, pctdistance=0.85, colors=color_palette, textprops={'fontsize': 11})
