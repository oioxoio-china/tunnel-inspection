...
# --- 1. 页面与样式配置 ---
st.set_page_config(
    page_title="隧道工程检验批划分系统 Pro v8.3",
    page_icon="🚇",
    layout="wide",
...
class InspectionCalculator:
    ...
    def calculate_single_tunnel(self, tunnel: Tunnel) -> Dict:
        ...
        for ic in ['02', '03']:
            for k in range(3):
                self._add_batch(results, tunnel.name, tunnel.id, '02', ic, k+1, '进洞口')
                self._add_batch(results, tunnel.name, tunnel.id, '02', ic, k+4, '出洞口')
        ...
        if trolley > 0:
            ...
            for r in range(rings):
                ...
                self._add_batch(results, tunnel.name, tunnel.id, '06', '01', r+1, '仰拱', start, end)
                self._add_batch(results, tunnel.name, tunnel.id, '06', '02', r+1, '拱墙', start, end)
...
def main():
    ...
    if page == "📋 参数配置":
        ...
        with col_seg:
            ...
            # --- 修复 2: 彻底放开所有属性的编辑权限，并加入容错处理 ---
            # 定义标准表头，防止空列表导致 data_editor 崩溃
            ...
            if not target_tunnel.segments:
                ...
                df_seg = pd.DataFrame(seg_data)[expected_columns] # 强制列顺序
            ...
            if st.button("💾 保存段落更改", type="primary"):
                new_segs = []
                # 强容错处理：用户新建空行时，赋予合法默认值防止崩溃
                for _, row in edited_df.iterrows():
                    def get_val(val, default): return default if pd.isna(val) else val
                    
                    start_m = float(get_val(row.get('起始(m)'), 0.0))
                    end_m = float(get_val(row.get('终止(m)'), 0.0))
                    ...
                    new_segs.append(TunnelSegment(
                        ...
                        end_mileage=end_m,
                        advance_per_cycle=float(get_val(row.get('进尺(m)'), 1.6)), 
                        lining_type=str(get_val(row.get('衬砌类型'), "")), 
                        steps=int(get_val(row.get('步骤数'), 2)),
                        frames_per_ring=int(get_val(row.get('榀数/环'), 2)),
                        frame_spacing=float(get_val(row.get('榀距(m)'), 0.8)),
    ...
    # ===== 页面：检验批计算 =====
        ...
        if 'last_result' in st.session_state:
            ...
            with c4: st.markdown(f'<div class="metric-card bg-orange"><div class="metric-label">二衬批数</div><div class="metric-value">{df_sum["衬砌"].sum():,}</div></div>', unsafe_allow_html=True)
